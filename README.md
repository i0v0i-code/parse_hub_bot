...
- 合并上游机器人 v2.1.16，解析库锁定 ParseHub 2.2.4。
- 小红书、抖音使用上游单作品解析，不启用本地扫码、凭据导出注入、平台冷却或主页批量。
- B站保留登录 Cookie、最高可用画质、个人空间/合集/多P任务、扫描缓存和阶段恢复；`/relogin` 仅提供 B站。
- 旧小红书/抖音主页任务暂停自动执行，`/tasks` 可查看历史记录；作品、归档、回传记录不删除。
- 主人私聊 WebDAV、群聊和其他用户仅回传的权限边界，以及 YouTube 归档/回传策略继续保留。
